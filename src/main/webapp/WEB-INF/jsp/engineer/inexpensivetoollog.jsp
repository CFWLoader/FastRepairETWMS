...
<%@ page import="pers.evan.fastrepair.service.ToolLogService" %>
<%@ page import="pers.evan.fastrepair.service.impl.ToolLogServiceImpl" %>
<%@ page import="pers.evan.fastrepair.model.*" %>
...
<%
    ...
    if(action != null && action.trim().equals("buildlog")){
        ...
        inexpensiveToolLog.setId(-1);
        inexpensiveToolLog.setEmployeeId(employee.getId());
        inexpensiveToolLog.setToolId(tool.getId());
        inexpensiveToolLog.setQuantity(0);
        ...
        inexpensiveToolLog = toolLogService.inexpensiveLogAssembler(
        ...
        int newId = toolLogService.addInexpensiveToolLog(inexpensiveToolLog);

        inexpensiveToolLog = toolLogService.getInexpensiveToolLogById(newId);
        ...
        */
        ...
        tool = toolService.getToolById(String.valueOf(inexpensiveToolLog.getToolId()));
        ...
        inexpensiveToolLog = toolLogService.getInexpensiveToolLogById(Integer.parseInt(request.getParameter("id").trim()));
    ...
    */
    if(tool == null || inexpensiveToolLog == null)throw new NullPointerException();
%>
            ...
            <div class="col-md-4">
                ...
                <div id="myTabContent" class="tab-content">
                    <div class="tab-pane active in" id="home">
                        <form id="tab" name="toolloginfo" action="inexpensivetoollog.jsp" method="post">

                            <input type="hidden" name="action" value="<%=action.equals("buildlog") ? "createlog" : "modifylog"%>"/>

                            <div class="form-group">
                                <label>Tool Log Id</label>
                                <input type="text" name="id" value="<%=inexpensiveToolLog.getId()%>" class="form-control" readonly>
                            </div>
                            <div class="form-group">
                                <label>Employee Name</label>
                                <input type="hidden" name="employeeid" value="<%=employee.getId()%>"/>
                                <input type="text" value="<%=employee.getFirstName() + " " + employee.getLastName()%>" class="form-control" readonly>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="quantity" value="<%=inexpensiveToolLog.getQuantity()%>" class="form-control" <%=action.equals("buildlog") ? "" : "readonly"%>>
                            </div>
                            <!--
                            <div class="form-group">
                                <label>Status</label>
                            ...
                            <div class="form-group">
                                <label>Status</label>
                                <select name="status" class="form-control" <%=inexpensiveToolLog.getStatus().equals("Waiting for deal")? "disabled=\"disabled\"" : ""%>>
                                    <option value="Waiting for deal" <%= inexpensiveToolLog.getStatus().equals("Waiting for deal")? "selected=\"selected\"" : ""%>>Waiting for deal
                                    </option>
                                    <option value="Lent" <%= inexpensiveToolLog.getStatus().equals("Lent")? "selected=\"selected\"" : ""%>>Lent
                                    </option>
                                    <option value="Broken" <%= inexpensiveToolLog.getStatus().equals("Broken")? "selected=\"selected\"" : ""%>>Broken
                                    </option>
                                </select>

                                <%=inexpensiveToolLog.getStatus().equals("Waiting for deal")? "<input type=\"hidden\" name=\"status\" value=\"" + inexpensiveToolLog.getStatus() + "\" />" : ""%>
                            </div>
                            <div class="form-group">
                                <label>Log Date</label>
                                <input type="hidden" name="logdate" value="<%=inexpensiveToolLog.getLogDate().getTime()%>"/>
                                <input type="text" value="<%=inexpensiveToolLog.getLogDate()%>" class="form-control" readonly>
                            </div>
                            <div class="form-group">
